...
const EpisodeDots: React.FC<{ opacity: number }> = ({ opacity }) => (
  <div style={{ display: "flex", gap: 14, alignItems: "center", opacity }}>
    {[true, false, false, false, false, false].map((active, i) => (
      <div key={i} style={{ width: active ? 32 : 10, height: 10, borderRadius: 5, backgroundColor: active ? C.gold : C.muted, opacity: active ? 1 : 0.4 }} />
    ))}
  </div>
);
...
export const Scene7CTA: React.FC<Props> = ({ durationInFrames }) => {
  ...
  return (
    <SceneBase bg={C.scene7bg}>
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 8, backgroundColor: C.gold, opacity: fadeIn(frame, 0, 20) }} />
      ...
      >
        <div style={{ opacity: seriesOpacity, fontFamily: BODY, fontWeight: 600, fontSize: 30, letterSpacing: "0.25em", color: C.muted, marginBottom: 12 }}>
          HISTORY OF TRUCKING
        </div>

        <div style={{ width: `${lineW * 520}px`, height: 4, backgroundColor: C.gold, marginBottom: 48 }} />

        <div style={{ opacity: mainOpacity, transform: `translateY(${mainY}px)`, fontFamily: HEADING, fontWeight: 700, fontSize: 92, lineHeight: 1.08, color: C.white, letterSpacing: "-0.01em", marginBottom: 32 }}>
          THE FULL STORY STARTS NOW
        </div>

        <div style={{ opacity: subOpacity, transform: `translateY(${subY}px)`, fontFamily: BODY, fontSize: 44, lineHeight: 1.5, color: C.whiteMuted, marginBottom: 52 }}>
          We're just getting started. Every era. Every legend. Every mile.
        </div>

        <div style={{ marginBottom: 36, opacity: dotsOpacity }}>
          <EpisodeDots opacity={1} />
        </div>

        <div style={{ opacity: ep2Opacity, transform: `translateY(${ep2Y}px)`, fontFamily: HEADING, fontWeight: 700, fontSize: 56, color: C.gold, letterSpacing: "0.04em", marginBottom: 48 }}>
        ...
        <div style={{ opacity: followOpacity, transform: `translateY(${followY}px)`, fontFamily: HEADING, fontWeight: 700, fontSize: 72, color: C.gold, letterSpacing: "-0.01em" }}>
          FOLLOW NOW
        </div>
      </div>
    </SceneBase>
